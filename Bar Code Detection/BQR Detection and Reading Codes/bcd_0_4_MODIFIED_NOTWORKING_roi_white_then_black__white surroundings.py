...
for cnt in contours:
    ...
    if 1500 < area < 90000:
     ...
     #   cv2.rectangle(gray,(x,y),(x+w,y+h),255,-1)
        ...
        white_roi = deepcopy(temp[y:y+h, x:x+w])
        gray_1 = cv2.cvtColor(white_roi,cv2.COLOR_BGR2GRAY)
        

        gray_xx = 255 - gray_1
        t = cv2.imread('C:\\Akhilesh\\BTech\\Inter_IIT_Tech_Meet_\'18\\Bar Codes\\b0006.jpg')
        stencil = np.zeros(t.shape).astype(t.dtype)
        color = [255, 255, 255]
        cv2.fillPoly(stencil, cnt, color)
        result = cv2.bitwise_and(t, stencil)
        cv2.drawContours(temp,[cnt],0,(255,255,0),1)
##        result = 255 - result
...
##                cv2.rectangle(gray_roi_2,(x,y),(x+w,y+h),255,-1)
                ...
                roi = deepcopy(orimg[by:by+bh, bx:bx+bw])
                if a == []:
                    a = deepcopy(roi)
                cv2.imshow('ROI',roi)
                cv2.waitKey(0)
