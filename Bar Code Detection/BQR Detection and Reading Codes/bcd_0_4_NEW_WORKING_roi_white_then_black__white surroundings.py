...
for cnt in contours:
    ...
    if 1500 < area < 90000:
     ...
     #   cv2.rectangle(gray,(x,y),(x+w,y+h),255,-1)
        ...
        temp = deepcopy(orimg)
        cv2.drawContours(temp,[cnt],0,(255,255,255),2)
        stencil = np.zeros(temp.shape).astype(temp.dtype)
        color = [255, 255, 255]
        cv2.fillPoly(stencil, [cnt], color)
        t1 = 255-temp
        result = cv2.bitwise_and(t1, stencil)
        result = 255-result
        ...
        white_roi = deepcopy(result[y:y+h, x:x+w])
        gray_1 = cv2.cvtColor(result,cv2.COLOR_BGR2GRAY)
        gray_roi_1 = cv2.dilate((255 - gray_1),kernel,iterations = 2)
